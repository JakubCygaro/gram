...
GRAM_DEFINE_FN(float, gram_update, size_t);
GRAM_DEFINE_FN(int, gram_get_draw_type);
GRAM_DEFINE_FN(size_t, gram_get_time_fn);
GRAM_DEFINE_FN(size_t, gram_get_dimensions_fn);
...
static void load_fns()
{
    ...
    TraceLog(LOG_INFO, "DRAW TYPE: %d", s_draw_type);
}
...
static void update_data()
{
    ...
    for (size_t t = 0; t < TIME; t++) {
        s_data[t] = gram_update(t);

        s_min = fmin(s_data[t], s_min);
        s_max = fmax(s_data[t], s_max);
    }
    s_full = s_max - s_min;
    s_colw = (float)PLOT_W / TIME;
